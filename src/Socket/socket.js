...
const SocketHandler = () => {
  ...
  useEffect(() => {
    ...
    socket.on("sendTargetAndMission", (target, mission) => {
      //A modifier
    ...
    socket.on("startGame", (target, mission, numberMission) => {
      // A modifier
      //dispatch(setListPlayer(updatedListPlayer));
      dispatch(setTarget(target));
      dispatch(setMission(mission));
    ...
    socket.on("isKilledConfirm", (target, mission) => {
      // A modifier
    ...
    socket.on("leaveGame", (target, mission) => {
      //A modifier
